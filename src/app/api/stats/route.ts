...
export async function GET() {
  ...
  // Order/customer counts are real — only the money figures are scaled by
  // this user's revenue_view_percent, set from the Users page.
  return NextResponse.json({
    ...
    perDay: perDay.map((r) => ({ date: r.day, amount: scaleAmount(r.amount, percent) })),
  });
}
